...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch):
        ...
        if not torch.isfinite(batch["img"]).all():
            LOGGER.warning("Non-finite values detected in validation batch['img']; sanitizing with torch.nan_to_num().")
            batch["img"] = torch.nan_to_num(batch["img"], nan=0.0, posinf=1.0, neginf=0.0)
        if self.args.temporal and "temporal_imgs" in batch:
            ...
            if not torch.isfinite(batch["temporal_imgs"]).all():
                LOGGER.warning(
                    "Non-finite values detected in validation batch['temporal_imgs']; "
                    "sanitizing with torch.nan_to_num()."
                )
                batch["temporal_imgs"] = torch.nan_to_num(batch["temporal_imgs"], nan=0.0, posinf=1.0, neginf=0.0)
        ...
        if self.args.temporal and "prev_img" in batch:
            ...
            if not torch.isfinite(batch["prev_img"]).all():
                LOGGER.warning(
                    "Non-finite values detected in validation batch['prev_img']; sanitizing with torch.nan_to_num()."
                )
                batch["prev_img"] = torch.nan_to_num(batch["prev_img"], nan=0.0, posinf=1.0, neginf=0.0)
