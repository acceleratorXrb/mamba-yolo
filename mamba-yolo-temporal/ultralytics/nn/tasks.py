...
def _tensor_nonfinite_stats(x):
    """Return finite/nan/inf stats for a tensor."""
    ...
    stats = {
        ...
        "nan_count": int(torch.isnan(x).sum().detach().cpu()),
        "posinf_count": int(torch.isposinf(x).sum().detach().cpu()),
        "neginf_count": int(torch.isneginf(x).sum().detach().cpu()),
    }
    if x.numel():
        safe_x = torch.nan_to_num(x.detach(), nan=0.0, posinf=0.0, neginf=0.0)
        stats["min"] = float(safe_x.amin().cpu())
        stats["max"] = float(safe_x.amax().cpu())
        stats["mean"] = float(safe_x.float().mean().cpu())
    return stats


def _nonfinite_summary(x):
    """Recursively summarize tensors contained in model inputs/outputs."""
    if isinstance(x, torch.Tensor):
        return _tensor_nonfinite_stats(x)
    if isinstance(x, (list, tuple)):
        return [_nonfinite_summary(v) for v in x]
    if isinstance(x, dict):
        return {str(k): _nonfinite_summary(v) for k, v in x.items()}
    return {"type": type(x).__name__, "repr": repr(x)[:200]}


def _contains_nonfinite(x):
    """Return True if any tensor inside x contains NaN/Inf."""
    if isinstance(x, torch.Tensor):
        return not bool(torch.isfinite(x).all().detach().cpu())
    if isinstance(x, (list, tuple)):
        return any(_contains_nonfinite(v) for v in x)
    if isinstance(x, dict):
        return any(_contains_nonfinite(v) for v in x.values())
    return False


def _sanitize_tensor_nonfinite(x, name, *, posinf=1.0, neginf=0.0):
    """Replace NaN/Inf in a tensor with bounded values and log once per call site."""
    if not isinstance(x, torch.Tensor) or torch.isfinite(x).all():
        return x
    LOGGER.warning(f"Non-finite values detected in {name}; sanitizing with torch.nan_to_num().")
    return torch.nan_to_num(x, nan=0.0, posinf=posinf, neginf=neginf)


def _json_safe(value):
    """Convert debug payloads to JSON-safe objects."""
...
def _write_nonfinite_forward_debug(model, module, module_input, module_output):
    """Persist the first module whose forward output becomes non-finite."""
    save_dir = Path(getattr(model, "_debug_save_dir", Path.cwd()))
    save_dir.mkdir(parents=True, exist_ok=True)
    context = getattr(model, "_debug_context", {}) or {}
    batch_i = context.get("batch_i", "unknown")
    epoch = context.get("epoch", "unknown")
    split = context.get("split", "unknown")
    debug_path = save_dir / (
        f"nonfinite_forward_{split}_epoch{epoch}_batch{batch_i}_module{getattr(module, 'i', 'unknown')}.json"
    )
    payload = {
        "split": split,
        "epoch": epoch,
        "batch_i": batch_i,
        "module_index": getattr(module, "i", None),
        "module_type": getattr(module, "type", module.__class__.__name__),
        "module_class": module.__class__.__name__,
        "module_from": getattr(module, "f", None),
        "input": _nonfinite_summary(module_input),
        "output": _nonfinite_summary(module_output),
    }
    debug_path.write_text(json.dumps(_json_safe(payload), indent=2, ensure_ascii=False))
    raise RuntimeError(
        "Non-finite forward activation detected at module "
        f"{getattr(module, 'i', 'unknown')} ({getattr(module, 'type', module.__class__.__name__)}). "
...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            module_input = x
            x = m(x)  # run
            if _contains_nonfinite(x):
                _write_nonfinite_forward_debug(self, m, module_input, x)
...
class TemporalDetectionModel(DetectionModel):
    """Three-frame temporal detection model inspired by the first MVP version."""
    ...
    def loss(self, batch, preds=None):
        ...
        if preds is None:
            preds = self.predict(
                batch["img"],
                temporal_imgs=batch.get("temporal_imgs"),
                temporal_valid=batch.get("temporal_valid"),
                prev_img=batch.get("prev_img"),
                has_prev=batch.get("has_prev"),
            )
        return self.criterion(preds, batch)
    ...
    def predict(
        ...
        x = _sanitize_tensor_nonfinite(x, "TemporalDetectionModel.predict(x)")
        if temporal_imgs is not None:
            temporal_imgs = _sanitize_tensor_nonfinite(temporal_imgs, "TemporalDetectionModel.predict(temporal_imgs)")
        if prev_img is not None:
            prev_img = _sanitize_tensor_nonfinite(prev_img, "TemporalDetectionModel.predict(prev_img)")
        if augment:
        ...
        return self._predict_temporal_clip(
            x, temporal_imgs, temporal_valid=temporal_valid, profile=profile, visualize=visualize
        )

    def _get_from(self, x, y, f):
    ...
    def _forward_to_detect(self, x, profile=False, visualize=False):
        ...
        x = _sanitize_tensor_nonfinite(x, "TemporalDetectionModel._forward_to_detect input")
        y, dt = [], []
        for m in self.model[:-1]:
            if m.f != -1:
                x = self._get_from(x, y, m.f)
            if profile:
                self._profile_one_layer(m, x, dt)
            module_input = x
            x = m(x)
    ...
    def _predict_temporal_clip(self, x, temporal_imgs, temporal_valid=None, profile=False, visualize=False):
        ...
        x = _sanitize_tensor_nonfinite(x, "TemporalDetectionModel._predict_temporal_clip center frame")
        temporal_imgs = _sanitize_tensor_nonfinite(
            temporal_imgs, "TemporalDetectionModel._predict_temporal_clip temporal_imgs"
        )
        ...
        for frame_idx in range(clip_length):
            if frame_idx == center_idx:
                frame_feats = current_feats
            else:
                frame_feats = self._forward_to_detect(temporal_imgs[:, frame_idx], profile=False, visualize=False)
            for scale_idx, feat in enumerate(frame_feats):
        ...
        if _contains_nonfinite(outputs):
            _write_nonfinite_forward_debug(self, detect, module_input, outputs)
        return outputs
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if embed and m.i in embed:
                ...
                if m.i == max(embed):
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        head = self.model[-1]
        module_input = [y[j] for j in head.f]
        x = head(module_input, batch)  # head inference
        if _contains_nonfinite(x):
            _write_nonfinite_forward_debug(self, head, module_input, x)
        return x
