...
TEMPORAL_ROOT="$ROOT/mamba-yolo-temporal"
OFFICIAL_ROOT="$ROOT/mamba-yolo-official"
VISDRONE_DATA_CFG="$ROOT/configs/datasets/VisDroneVID_local.yaml"
UAVDT_DATA_CFG="$ROOT/configs/datasets/UAVDT_full.yaml"
UAVDT_BENCH_DATA_CFG="$ROOT/configs/datasets/UAVDT_full_benchmark.yaml"
...
命令:
  ...
  train-visdrone-singleframe 启动官方原始 Mamba-YOLO 单帧 VisDrone-VID baseline
...
latest_main_visdrone_best() {
  find "$ROOT/output_dir" \
    \( -path "$ROOT/output_dir/visdrone_vid_temporal/*/weights/best.pt" -o -path "$ROOT/output_dir/visdrone_vid_official/*/weights/best.pt" \) \
...
setup_env() {
  ...
  cd "$TEMPORAL_ROOT/selective_scan"
...
write_uavdt_benchmark_yaml() {
  mkdir -p "$ROOT/configs/datasets"
  cat > "$UAVDT_BENCH_DATA_CFG" <<EOF
...
postflight_summary() {
  ...
  if [[ -f "$visdrone_yaml" && -d "$ROOT/data/processed/VisDroneVID/val/images" ]]; then
    status_line "VisDrone val" "OK" "$ROOT/data/processed/VisDroneVID/val/images"
  else
    status_line "VisDrone val" "MISSING" "$ROOT/data/processed/VisDroneVID/val/images"
  fi
...
train_uavdt_temporal() {
  cd "$ROOT"
  exec "$PYTHON_BIN" "$TEMPORAL_ROOT/mbyolo_train.py" \
    --task train \
    --config ultralytics/cfg/models/mamba-yolo/Mamba-YOLO-T.yaml \
    --data "$UAVDT_BENCH_DATA_CFG" \
...
eval_visdrone() {
  ...
  local eval_py_path="$TEMPORAL_ROOT"
  if [[ "$weights_path" == *"/output_dir/visdrone_vid_official/"* ]]; then
    eval_py_path="$OFFICIAL_ROOT"
  fi
  local run_name
  run_name="$(basename "$(dirname "$(dirname "$weights_path")")")"
  exec env PYTHONPATH="$eval_py_path${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" scripts/evaluate_visdrone_vid.py \
...
post_eval_main_visdrone() {
  ...
  if env PYTHONPATH="$eval_py_path${PYTHONPATH:+:$PYTHONPATH}" "$PYTHON_BIN" "$ROOT/scripts/evaluate_visdrone_vid.py" \
    --weights "$weights_path" \
    --data "$VISDRONE_DATA_CFG" \
    --split val \
    --device 0 \
