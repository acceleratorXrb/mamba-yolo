...
def safe_target_scores_sum(target_scores: torch.Tensor) -> torch.Tensor:
    """Reduce target scores to a finite positive scalar used as a loss normalizer."""
    score_sum = torch.nan_to_num(target_scores.sum(), nan=0.0, posinf=0.0, neginf=0.0)
    return score_sum.clamp_min(1.0)


def tensor_debug_summary(x: torch.Tensor | None):
    """Return finite/range summary for a tensor without dumping full contents."""
    if x is None:
        return None
    summary = {
        "shape": list(x.shape),
    ...
    if x.numel():
        ...
        summary["min"] = float(x_safe.amin().cpu())
        summary["max"] = float(x_safe.amax().cpu())
        summary["mean"] = float(x_safe.mean().cpu())
    return summary
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        self._last_debug = {
            "weight": tensor_debug_summary(weight),
            "iou": tensor_debug_summary(iou),
            "loss_iou_raw": tensor_debug_summary(loss_iou.view(1)),
            "loss_dfl_raw": tensor_debug_summary(loss_dfl.view(1)),
        }
...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        target_scores_sum = safe_target_scores_sum(target_scores)

        # Cls loss
        # loss[1] = self.varifocal_loss(pred_scores, target_scores, target_labels) / target_scores_sum  # VFL way
        cls_bce = self.bce(pred_scores, target_scores.to(dtype))
        loss[1] = cls_bce.sum() / target_scores_sum  # BCE
        ...
        if fg_mask.sum():
            ...
            bbox_debug = getattr(self.bbox_loss, "_last_debug", None)
        else:
            bbox_debug = None
        ...
        self._last_loss_debug = {
            ...
            "cls_bce": tensor_debug_summary(cls_bce),
            "bbox_branch": bbox_debug,
            "pred_scores": tensor_debug_summary(pred_scores),
            "pred_distri": tensor_debug_summary(pred_distri),
            "pred_bboxes": tensor_debug_summary(pred_bboxes),
            "target_bboxes": tensor_debug_summary(target_bboxes),
            "target_scores": tensor_debug_summary(target_scores),
        }

        if not torch.isfinite(loss).all() and not self._logged_nonfinite_loss:
            self._logged_nonfinite_loss = True
            debug = dict(self._last_loss_debug)
            debug.update({
                "loss": loss.detach().cpu().tolist(),
                "batch_size": int(batch_size),
                "imgsz": [int(x) for x in imgsz.detach().cpu().tolist()],
                "anchor_points_finite": bool(torch.isfinite(anchor_points).all().detach().cpu()),
                "stride_tensor_finite": bool(torch.isfinite(stride_tensor).all().detach().cpu()),
                "gt_labels_finite": bool(torch.isfinite(gt_labels).all().detach().cpu()),
                "gt_labels": tensor_debug_summary(gt_labels),
            })
