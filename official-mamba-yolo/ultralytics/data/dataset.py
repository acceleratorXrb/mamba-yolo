...
class YOLOTemporalDataset(YOLODataset):
    """YOLO dataset with odd-length temporal clips centered on the current frame."""

    frame_pattern = re.compile(r"^(?P<prefix>.+_img)(?P<frame>\d+)(?P<suffix>\.[^.]+)$")

    def __init__(self, *args, temporal_stride=1, temporal_clip_length=3, **kwargs):
        self.temporal_stride = max(int(temporal_stride), 1)
        self.temporal_clip_length = max(int(temporal_clip_length), 1)
        if self.temporal_clip_length % 2 == 0:
            raise ValueError("temporal_clip_length must be odd.")
        self.temporal_offsets = [
            (idx - self.temporal_clip_length // 2) * self.temporal_stride for idx in range(self.temporal_clip_length)
        ]
        super().__init__(*args, **kwargs)
    ...
    def _resolve_temporal_im_files(self, im_file: str) -> list[str]:
        ...
        if match is None:
            return [str(path)] * self.temporal_clip_length
    ...
    def get_image_and_label(self, index):
        ...
        clip_im_files = self.temporal_im_files[index]
        current_im_file = str(label["im_file"])
        temporal_imgs = []
        temporal_valid = []
        ...
        for clip_idx, clip_im_file in enumerate(clip_im_files):
            if clip_im_file == current_im_file:
                ...
                continue
            clip_img = cv2.imread(clip_im_file)
            if clip_img is None:
                temporal_imgs.append(label["img"].copy())
                temporal_valid.append(0.0)
            else:
                temporal_imgs.append(clip_img)
                temporal_valid.append(1.0)

        label["temporal_imgs"] = temporal_imgs
        label["temporal_im_files"] = clip_im_files
        label["temporal_valid"] = np.asarray(temporal_valid, dtype=np.float32)
        label["sequence_id"] = Path(label["im_file"]).stem.split("_img")[0]
        return label
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "temporal_imgs", "temporal_valid"}:
                value = torch.stack(value, 0)
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb"}:
                value = torch.cat(value, 0)
            new_batch[k] = value
